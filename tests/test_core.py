...
class TestArrow:
    ...
    def test_table_small_uses_native_repr(self):
        table = self.pa.table({"a": [1, 2], "b": [3, 4]})
        r = reprobate.render(table, 500)
        assert r == repr(table)

    def test_table_compact_fallback(self):
        table = self.pa.table({f"col_{i}": range(100) for i in range(10)})
        r = reprobate.render(table, 80)
        assert "Table" in r
        assert "100x10" in r

    def test_table_budget_respected(self):
        table = self.pa.table({f"col_{i}": range(100) for i in range(10)})
        for budget in [5, 10, 20, 50, 100, 200]:
    ...
    def test_array_truncation(self):
        arr = self.pa.array(range(1000))
        r = reprobate.render(arr, 60)
        assert len(r) <= 60
        assert "Array" in r
        assert "more" in r

    def test_array_budget_respected(self):
        arr = self.pa.array(range(100))
        for budget in [5, 10, 20, 50, 100, 200]:
    ...
    def test_chunked_array_small_uses_native_repr(self):
        chunked = self.pa.chunked_array([[1, 2], [3, 4]])
        r = reprobate.render(chunked, 500)
        assert r == repr(chunked)

    def test_chunked_array_compact_fallback(self):
        chunked = self.pa.chunked_array([list(range(1000))])
        r = reprobate.render(chunked, 40)
        assert "ChunkedArray" in r
        assert "int64" in r

    def test_chunked_array_budget_respected(self):
        chunked = self.pa.chunked_array([list(range(1000))])
        for budget in [5, 10, 20, 50, 100, 200]:
...
class TestPolars:
    ...
    def test_dataframe_small_uses_native_repr(self):
        df = self.pl.DataFrame({"a": [1, 2], "b": [3, 4]})
        r = reprobate.render(df, 200)
        assert r == repr(df)

    def test_dataframe_compact_fallback(self):
        df = self.pl.DataFrame({"a": [1, 2], "b": [3, 4]})
    ...
    def test_dataframe_many_columns_truncation(self):
        df = self.pl.DataFrame({f"col_{i}": range(5) for i in range(20)})
    ...
    def test_dataframe_budget_respected(self):
        df = self.pl.DataFrame({f"col_{i}": range(100) for i in range(20)})
        for budget in [5, 10, 20, 50, 100, 200]:
            r = reprobate.render(df, budget)
            assert len(r) <= budget, (
                f"Budget {budget} exceeded: got {len(r)} chars: {r!r}"
    ...
    def test_series(self):
        s = self.pl.Series("vals", [1, 2, 3])
        r = reprobate.render(s, 200)
        assert "Series" in r
        assert "3" in r
        assert "vals" in r

    def test_series_budget_respected(self):
        s = self.pl.Series("vals", range(1000))
        for budget in [5, 10, 20, 50, 100, 200]:
